...
export const signIn = createAsyncThunk('auth/login',async (data, { rejectWithValue }) => {
	...
	try {
		...
		return rejectWithValue(err.response?.data?.message || 'Please, check your credentials and try again')
	}
})

export const signUp = createAsyncThunk('auth/register',async (data, { rejectWithValue }) => {
	const { firstname, username, password } = data
	const response = await AuthService.register({ firstname, username, password })
	if (response.token) {
		const token = response.token;
